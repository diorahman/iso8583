...
ISO8583.prototype.parseData = function() {
  var input = '';
  if (typeof this.builderData[1] != 'undefined' && this.builderData[1] == '?')
    input = this.iso.substr(4+32, this.iso.length - 4 - 32);
  else
    input = this.iso.substr(4+16, this.iso.length - 4 - 16);  
  
  if (Object.keys(this.builderData).length == 0)
    return;
  ...
  for (var key in this.builderData) {
    ...
    if (DataElement[key][0] != 'b') {
      if (DataElement[key][2] == FIXED_LENGTH) {
        ...
        if (tmp.length == DataElement[key][1]) {
          if (DataElement[key][0] == 'n') {
            this.builderData[key] = input.substr(0, DataElement[key][1]); 
          } else {
            this.builderData[key] = input.substr(0, DataElement[key][1]).trimLeft();
          }
        ...
        if (len == tmp.length) {
          ...
          if (tmp2.length == num) {
            if (DataElement[key][0] == 'n') {
              this.builderData[key] = parseFloat(tmp2);
            } else {
              this.builderData[key] = tmp2.trimLeft();
            }
      ...
      if (key > 1) {
        ...
        if (tmp.length == 16) {
          this.builderData[key] = this.iso.substr(4 + 16, 16);
          this.valid['de'][key] = true;
        }
      }
    }
